...
async function montarPlantilla(): Promise<void> {
  ...
  aplicarModo() // engancha la selección de vectores al nuevo svg + construye overlays
...
//  - 'completa': UNA sola superficie. Los overlays normales (texto/foto/agregados)
//    siguen activos Y además la selección de vectores está SIEMPRE enganchada al
//    svg (sin la clase .modo-grafico, que apagaría los overlays). Así un clic en
//    una gráfica suelta de la plantilla la selecciona, sin togglear "Gráficos".
//  - 'plantilla': solo overlays filtrados (texto + foto), sin selección de vectores.
function aplicarModo(): void {
  ...
  // Re-set idempotente de los listeners de selección (el svg pudo haber cambiado).
...
function grafKey(e: KeyboardEvent): void {
  if (e.key === 'Escape') {
    ...
    // En completa la selección de vectores está siempre activa: Escape solo deselecciona.
...
async function aplicarSnapshot(p: Proyecto): Promise<void> {
  ...
  suprimirHistorial = true
  aplicarModo() // re-engancha la selección al nuevo svg + fija modo + overlays
  suprimirHistorial = false
}
